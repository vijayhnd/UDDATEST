...
interface G_CounterCustomBetViewProps extends AppValidationComponentProps {
...
interface G_CounterCustomBetViewState extends AppValidationComponentState {
...
class G_CounterCustomBet extends AppValidationComponent<G_CounterCustomBetViewProps, G_CounterCustomBetViewState>
{
  ...
  async componentDidMount() {
    ...
    let encrypted_bet_id = navigation.getParam('encrypted_bet_id');
    this.encrypted_bet_id = encrypted_bet_id;
    // this.encrypted_bet_id = 'QXgrTmZKUWlTa2xoeWJPdmdUQmRpQT09';
    console.log('encrypted_bet_id', encrypted_bet_id);
    this.betDetailData();
  }
  betDetailData() {
    var url = UrlService.CONSTURI + 'index.php/' + UrlService.APIVERSION3 + '/apiGaming/share_custom_propbet_info/' + this.encrypted_bet_id + '/custom_prop';
    ...
    fetch(url, {
      ...
      .then((responseJson) => {
        ...
        that.setState({ bet_id: responseJson.data.bet_detail.bet_id })
        ...
        that.setState({ bet_type: responseJson.data.bet_detail.bet_reject_type });
        that.setState({ opposite: responseJson.data.opposite });
        that.setState({ customBetTest: responseJson.data.bet_detail.question });
        that.setState({ customBetQTest1: responseJson.data.bet_detail.answer });
        that.setState({ customBetQTest2: responseJson.data.opposite.answer });
        if (responseJson.data.opposite.odds_type == 'selected') {
          that.setState({ customBetOdds: responseJson.data.bet_detail.odds });
          that.setState({ isEditable: false });
        } else {
          that.setState({ isEditable: true });
          that.setState({ customBetOdds: 'CUSTOM' });
          that.setState({ customBetCutmOdds: responseJson.data.bet_detail.odds });


        }
        that.setState({ betenable: true })
        ...
        var formated_date = moment(new_time_stampExp).format('MM-DD-YYYY');
        that.setState({ customBetDate: formated_date });
        that.setState({ customBetTime: formated_timeExp });


        console.log('formated_timeExp', formated_timeExp);
        console.log('batdateExp', batdateExp);
        console.log('Match_dateExp', Match_dateExp);
        console.log('zonevalueExp', zoneExp);

        /**
         * Settle bet by
         */

        var seasonnew_time_stampExp = this.state.opposite.season_expired_timestamp * 1000;
        // that.setState({ bet_type: responseJson.data.bet_detail.bet_reject_type }) ;
        // that.setState({ opposite: responseJson.data.opposite });
        var formated_seasontimeExp = moment(seasonnew_time_stampExp).format('LT');
        var seasonbatdateExp: any = moment(seasonnew_time_stampExp).format('LL');
        var seasonMatch_dateExp: any = new Date(seasonnew_time_stampExp).toString().split(' ');
        var seasonzonevalueExp: any = seasonMatch_dateExp[6].toString();
        var seasonzoneExp: any = seasonzonevalueExp.substr(1, seasonzonevalueExp.length - 2);
        var seasonformated_date = moment(seasonnew_time_stampExp).format('MM-DD-YYYY');
        that.setState({ settleBetDate: seasonformated_date });
        that.setState({ settleBetTime: formated_seasontimeExp });
  ...
  CustomBetDialog(item: any) {
    ...
    console.log(this.state.SelectedBetOption);
  ...
  async shareOption(item: any, bettype: any) {
    ...
    if (bettype == 'CUSTOM') {
      ...
      try {
        ...
        MessageString += '"';
        MessageString += this.state.oddsValue + '" odds that "' + this.state.customBetTest + '". I’ll put up "' + this.state.POSTamounttowin.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' UDDA" bucks to your "' + this.state.POSTBetAmount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' UDDA" bucks, you can accept all of or any part of the bet.\nMy answer "' + this.state.customBetQTest1 + '"\nBet closes at  "' + betTimeExpr + ', ' + batdateExp + '".'
      }
      catch (e) {
  ...
  componentWillReceiveProps(nextProps: G_CounterCustomBetViewProps) {
    if (nextProps.listeners!.includes(this.constructor.name)) {
      ...
      else if (nextProps.serviceKey === ServiceKeys.CounterCustomBet) {  // Counter  Custom bet response 
        switch (nextProps.requestStatus) {
          case ServiceRequestStatus.FinishedWithSuccess:
            this.serviceRequestInProgress = false;
            console.log("counterbetaFriendResponse " + JSON.stringify(nextProps.customBetaFriendResponse));
            var response = nextProps.customBetaFriendResponse!.response;
            if (response.message == 'success') {
              RouterBuilder.resetRouteTo(AppScreens.Gambling_ApplicationStack, this.props);return;
              var amount_to_win;
          ...
          <Animated.View style={[{ flex: 1 }, { transform: [{ translateY: this.state.shift }] }]}>
            ...
            <View style={styles.customhead}>              
              ...
              <View style={{ alignContent: 'flex-start', alignItems: 'flex-start', width:'75%' }}>
                <Text style={[styles.customheadtext,{color:'#68bcbc',marginBottom:5}]}>CREATE COUNTER BET</Text>
              </View>
                ...
                <View style={{ width: '48%' }}>
                  ...
                  <View style={{ height: hp(8.1), flexDirection: 'row', width: '100%', borderColor: '#cfcfcf', borderWidth: 1, borderRadius: 3, }}>
                    ...
                    <Dropdown
                      ...
                      onChangeText={(text) => { this.onChangeOdd(text) }}
                      fontSize={hp(1.8)}
                      value={this.state.customBetOdds}
...
export default connect(mapStateToProps)(G_CounterCustomBet);
