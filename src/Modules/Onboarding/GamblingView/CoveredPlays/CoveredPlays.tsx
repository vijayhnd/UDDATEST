...
import { View, Text, Image, ScrollView, FlatList, Animated, TouchableWithoutFeedback, Share, BackHandler, Alert } from "react-native";
...
interface G_CoveredPlaysViewProps extends AppValidationComponentProps {
}

interface G_CoveredPlaysViewState extends AppValidationComponentState {
    DataList?: any;
    isshow: any;
    loader: any;
...
export default class G_CoveredPlaysView extends AppValidationComponent<G_CoveredPlaysViewProps, G_CoveredPlaysViewState>
    ...
    constructor(props: G_CoveredPlaysViewProps) {
        super(props);
        this.state = {
            DataList: [],
            isshow: true,
    ...
    private loadMatches(week: number) {
        switch (week) {
            case 1: {
                this.setState({ DataList: playTable.data });
            } break;
    ...
    callMethod(CoverPlayVersion: any) {

        this.setState({ loader: true }); 
        var url;
        if (CoverPlayVersion == 1) {
            url = UrlService.CONSTURI +'index.php/'+ UrlService.APIVERSION3 +'/api/covered_play';
        }
        else {
            url = UrlService.CONSTURI +'index.php/'+ UrlService.APIVERSION3 +'/api/covered_play/' + CoverPlayVersion;
        }
        ...
        fetch(url, {
            ...
            .then((responseJson) => {
                ...
                console.log('Success covered play' + JSON.stringify(responseJson));
    ...
    CoveredPlaySwitchValueChanged() {
        if (this.state.CoveredPlaySwitchAccepted === true) {
            this.setState({ CoveredPlaySwitchAccepted: false });
            this.callMethod('1');
        }
        else {
            this.setState({ CoveredPlaySwitchAccepted: true });
            this.callMethod('2');
        }
                        ...
                        <View style={{ flex: 1 }}>
                            <View style={{ flexShrink: 1, backgroundColor: 'white' }}>
                                ...
                                <View style={styles.NameHeaderContainer}>
                                    <View style={{ width: '70%' }}>
                                        <Text style={styles.NameStyle}>COVERED PLAYS</Text>
                                    </View>
                                    <View style={{ width: '30%', alignItems: 'center', flexDirection: 'row', }}>

                                        <TouchableWithoutFeedback onPress={() => { this.CoveredPlaySwitchValueChanged() }}>
                                            <Image source={this.state.CoveredPlaySwitchAccepted === true ? require('../../../../images/toggle_on.png') : require('../../../../images/toggle_off.png')}
                                    ...
                                    this.state.DataList.length > 0 ?
                                        ...
                                        <View style={styles.OtherTextContainer}>
                                            <View style={styles.OtherTextSubContainer}>
                                                <Text style={styles.UnderConstText}>No one's covered</Text>
                                                <Text style={styles.UnderConstText}>your action yet.</Text>
