...
import RefferDialog from "../../../../Components/CustomComponents/RefferDialog/RefferDialog";
import Circle_Image from "../../../../Components/CustomComponents/CircleImage/circle_image_dashboard";
...
class DashboardView extends AppValidationComponent<G_DashboardViewProps, G_DashboardViewState>
  ...
  private jsonData = Application.sharedApplication().user!.profile.versionData;
  ...
  Update_notnow:''
   ...
   changeBet(index,value){
     var data =  this.state.betArray
     var bet= this.state.betArray
     bet.map((item,i)=>{
       if(i==index)
       {
...
async openoverlay(){
  ...
  try {
    ...
    guest_user = await AsyncStorage.getItem('guest_dialog');
    console.log('guest_user response',guest_user)
    if(guest_user == 'true')
    {
      that.setState({guestUserDialog:false})
      
    }else{
      that.setState({guestUserDialog:true})
      }
...
async componentDidMount() {
  ...
  // this.updateAppNotice('api')
  if (Application.sharedApplication().user!.profile.userType == null && Application.sharedApplication().user!.profile.userType != 'Guest') {
  ...
  this.setState({ DataList: [], startPage: 1 });
  ...
  if (Platform.OS === 'ios') {
    ...
    else if (Application.sharedApplication().DeeplinkName == 'contestbet/' || Application.sharedApplication().DeeplinkName == 'contestbetnew/') {
      this.setState({ aceeptoverlay: false });
    ...
    else if (Application.sharedApplication().DeeplinkName == 'watchvideos/') {
      this.setState({ aceeptoverlay: false });
      //this.props.navigation!.navigate(AppScreens.G_ImageZoom);
	...
	else if (Application.sharedApplication().DeeplinkName == 'referandearn/') {
      this.setState({ aceeptoverlay: false });
      //this.props.navigation!.navigate(AppScreens.G_ImageZoom);
	...
	else if (Application.sharedApplication().DeeplinkName == 'spinandwin/') {
    ...
    else if (Application.sharedApplication().DeeplinkName == 'index.php?contestbet/' || Application.sharedApplication().DeeplinkName == 'index.php?contestbetnew/') {
      this.setState({ aceeptoverlay: false });
      this.props.navigation!.navigate(AppScreens.G_InvitationToJoin);
    }
    ...
    else if (Application.sharedApplication().DeeplinkName == 'index.php?watchvideos/') {
      this.setState({ aceeptoverlay: false });
      //this.props.navigation!.navigate(AppScreens.G_ImageZoom);
	...
	else if (Application.sharedApplication().DeeplinkName == 'index.php?referandearn/') {
      this.setState({ aceeptoverlay: false });
      //this.props.navigation!.navigate(AppScreens.G_ImageZoom);
...
else if (Application.sharedApplication().DeeplinkName == 'index.php?spinandwin/') {
 ...
 async callReffer_dialog(){
    if (Application.sharedApplication().user!.profile.userType==null && Application.sharedApplication().user!.profile.userType != 'Guest') {
     
      var that = this;
      try {
       
       let  reffer_dialog = await AsyncStorage.getItem('reffer_dialog');
        let reffer_dialogall = await AsyncStorage.getItem('reffer_dialogall');
        console.log('guest_user response',reffer_dialog)

        if(reffer_dialogall ==null)
        {
          if(reffer_dialog ==null)
          {
            that.setState({ReferDialog:true}) 
  
          }else{
            that.setState({ReferDialog:false})
            }
      ...
      //  this.setState({ReferDialog:true})
     }
  }
  ...
  updateUserToken()
  {
    ...
    fetch(UrlService.CONSTURI + 'index.php/'+ UrlService.APIVERSION3 +'/api/update_guest_device', {
      method: 'POST',
      headers: {
      ...
      .then((responseJson) => {
        ...
        }else
       {
        ...
        console.log('token upadted' + JSON.stringify(this.state.customProbBetList));
 ...
 async getBanner() {
    ...
    that.setState({ loader: true });
      ...
      fetch(UrlService.CONSTURI + 'index.php/' + UrlService.APIVERSION3 + '/banner/get_list', {
        method: 'GET',
      }).then((response) => response.json())
        .then((responseJson) => {
          that.setState({ loader: false });
          ...
          if (responseJson.data.length != 0) {
            this.savebanner(responseJson.data,responseJson.video)
            this.setState({slider_image_array:responseJson.data})
            this.setState({video_list:responseJson.video})
            this.setState({banner_time:parseInt(responseJson.banner_time)})
            console.log('getBanner Display', responseJson.data);
 ...
 async savebanner(data:any,video:any){
  try {
    await AsyncStorage.setItem('banner_data',JSON.stringify(data));
    await AsyncStorage.setItem('banner_video_data',JSON.stringify(video));
  } catch (error) {
    // Error retrieving data
    console.log(error.message);
  }
  ...
  pressSlider(index){
// console.log('slider image_array', this.state.image_array)
// console.log('slider index value', index)
// console.log('slider whole data by index', this.state.slider_image_array[index])
// console.log('slider whole data', this.state.slider_image_array)

if(this.state.slider_image_array[index].banner_title=='Refer & Earn ')
{
  this.props.navigation!.navigate(AppScreens.G_ReferEarnView)
  // this.props.navigation!.navigate(AppScreens.G_Yotube)
}else if(this.state.slider_image_array[index].banner_title=='Video Tutorial')
{
  // this.props.navigation!.navigate(AppScreens.G_ReferEarnView)
  if(this.state.video_list.length==1)
  {
    this.props.navigation!.navigate(AppScreens.G_Yotube, { params: { video_id: this.state.video_list[0].video_link} })
  }else{
    this.props.navigation!.navigate(AppScreens.G_VideoList)
  }
...
CleverTap.profileGetCleverTapAttributionIdentifier((err, res) => { 
  ...
  appsFlyer.onInstallConversionData((res) => { 
    ...
    if (league_id==""){
      ...
      // this.setState({ guest2: true });
      // this.setState({ guestUserDialog: true });
      //  this.props.navigation!.navigate(AppScreens.G_Scaner);

     // this.props.navigation!.navigate(AppScreens.G_BetInfo);
    //  this.props.navigation!.navigate(AppScreens.G_Settlepool);
      // this.props.navigation!.navigate(AppScreens.G_ImageZoom);
      // this.props.navigation!.navigate(AppScreens.G_CustomBet);
      return;
    }
    } else if (league_id=="bingo"){
      this.setState({ showProBasketBall: false });//ashish
      this.setState({ showFighting: false }); //garima
      this.setState({ showSoccer: false });
      this.setState({ showAutoRacing: false }); //vijay
    ...
    if (league_id == 10) {
      if (value == "") {
       // alert('welcome')
        ...
        // console.log('ashish team data',teamData)
        // this.data3 = teamData
        //  this.setState({ selectedValue: teamData[0].value });
       await  this.setState({ selectedValue: 'all' });
       }
       console.log('ashish team value',value)
      ...
      this.groupType = this.state.selectedValue
      ...
      if (value == "") {
        ...
        // //  this.setState({ legueArrayTeamData: teamData });
        // console.log('ashish team data',teamData)
        // this.dataSoccer = teamData
        await  this.setState({ selectedSoccerValue: 'all' });
       }
      ...
      this.setState({ showClgFootball: false }); //vijay
      this.setState({ showProBasketBall: false }); //vijay
      this.groupType = this.state.selectedSoccerValue
      ...
      if (value == "") {
        ...
        // //  this.setState({ legueArrayTeamData: teamData });
        // console.log('ashish team data',teamData)
        // this.dataAutoRacing = teamData
        await  this.setState({ selectedAutoRacingValue: 'all' });
       }
      this.setState({ showFighting: false });
      this.setState({ showSoccer: false });
      ...
      this.setState({ showProBasketBall: false }); //vijay
      //this.setState({ selectedValue: 'UFC' });//garima
      this.groupType = this.state.selectedAutoRacingValue
      ...
      if (value == "") {
        ...
        // //  this.setState({ legueArrayTeamData: teamData });
        // console.log('ashish team data',teamData)
        // this.dataHorseRacing = teamData
        await  this.setState({ selectedHorseRacingValue: 'all' });
       }
      this.setState({ showFighting: false });
      this.setState({ showSoccer: false });
      this.setState({ showAutoRacing: false }); //vijay
      ...
      this.setState({ showProBasketBall: false }); //vijay
      //this.setState({ selectedValue: 'UFC' });//garima
      this.groupType = this.state.selectedHorseRacingValue
      ...
      //  console.log('ashish team data',teamData)
      //   this.setState({ legueArrayTeamData: teamData });
      await  this.setState({ dashboard_team_id: 'all' });
      }
      this.setState({ showFighting: false });
      this.setState({ showSoccer: false });
      this.setState({ showAutoRacing: false }); //vijay
      ...
      //this.setState({ selectedValue: 'UFC' });//garima
      this.groupType = this.state.dashboard_team_id
    } else if (league_id == 3 || league_id == 8 || league_id == 'all') {
      if (value == "") {
        // var teamData = await this.call_team_list_by_league(league_id, value);
        // console.log('ashish team data',teamData)
        // this.setState({ legueArrayTeamData: teamData });
      ...
      this.setState({ showProBasketBall: true }); //vijay
      //this.setState({ selectedValue: 'UFC' });//garima
      if(this.state.dashboard_team_id=='all')
     {
        this.groupType ='all'
      }else{
        this.groupType =''
    ...
    { console.log('numberof rows condition  :  true ')
      var params: any = {
      'league_id': league_id=='all'?3:league_id,
      // 'league_id': league_id,
      'group_type':league_id=='all'?'all':this.groupType,
...
console.log( this.authorisationToken);
    ...
    fetch(UrlService.CONSTURI + 'index.php/'+ UrlService.APIVERSION3 +'/apiGaming/dashboard', {
      ...
      {console.log('overlaylocal true')
        ...
        if (value == "") {
       this.callAllData(league_id,value)
        }
      }
...
async callAllData(league_id:any,value:any){
  if (league_id == 10) {
    if (value == "") {
      var teamData = await this.call_team_list_by_league(league_id, value);
      //  this.setState({ legueArrayTeamData: teamData });
      ...
      if (value == "") {
        ...
        console.log('ashish team data',teamData)
        this.dataSoccer = teamData
        // this.setState({ selectedSoccerValue: teamData[0].value });
       }}else if (league_id == 14) {
        if (value == "") {
          var teamData = await this.call_team_list_by_league(league_id, value);
          //  this.setState({ legueArrayTeamData: teamData });
          console.log('ashish team data',teamData)
          this.dataAutoRacing = teamData
          // this.setState({ selectedAutoRacingValue: teamData[0].value });
         }}else if (league_id == 22) {
          if (value == "") {
            var teamData = await this.call_team_list_by_league(league_id, value);
            //  this.setState({ legueArrayTeamData: teamData });
            console.log('ashish team data',teamData)
            this.dataHorseRacing = teamData
             //this.setState({ selectedHorseRacingValue: teamData[0].value });
           }}else if (league_id == 2) {
            if (value == "") {
             var teamData = await this.call_team_list_by_league(league_id, value);
             console.log('ashish team data',teamData)
  ...
  prev_next_dashboard_API_call(currentindex: any, flagofindex: any, count: any, currIndVal: any,isloadMore: any) {
    ...
    if (currentindex == 0) {
      ...
      if (this.state.selected_Legue_id == 10) {
        ...
        this.setState({ showClgFootball: false }); //vijay
        //this.setState({ selectedValue: 'UFC' });//garima
        this.groupType = this.state.selecteValue
   ...
   if(numberofRows=='All')
   {
    if (isloadMore) {
      var params: any = {
        'league_id': this.state.selected_Legue_id=='all'?3:this.state.selected_Legue_id,
        // 'league_id': this.state.selected_Legue_id,
        'dashboard': currIndVal,//currentindex,
        'group_type': this.state.selected_Legue_id=='all'?'all':this.groupType,
        // 'group_type': this.groupType,
        'start': '',
        'per_page': '',
      ...
      if (nextProps.serviceKey === ServiceKeys.GetProfileServiceName) {
        switch (nextProps.requestStatus) {
          case ServiceRequestStatus.FinishedWithSuccess:
            ...
            this.jsonData=nextProps.response.userProfile.versionData
            if((UrlService.APPVERSION_iOS <  nextProps.response.userProfile.versionData.ios_version && Platform.OS == 'ios') || (UrlService.APPVERSION_ANDROID <   nextProps.response.userProfile.versionData.android_version && Platform.OS == 'android'))
        {
            this.updateAppNotice('api')
        }
        ...
        switch (nextProps.requestStatus) {
          case ServiceRequestStatus.FinishedWithSuccess:
            ...
            console.log('check >>>>123'+JSON.stringify(this.state.selectedItem));
  ...
  submitFeedbackAPI() {
    ...
    if (this.state.FeedbackSubject == '') {
      ...
      this.setState({submitfeedbackbtn:false});
...
async  updateAppNotice(value:any){

    console.log('user version data ashish',this.jsonData)
  ...
  if(jsonData.hide_update_popup == "0")
  {
    if( (jsonData.force_android == "0" && Platform.OS == 'android') || (jsonData.force_ios== "0" && Platform.OS == 'ios')  ){
      ...
      this.setState({Update_msg:(Platform.OS == 'ios') ? jsonData.ios_msg : jsonData.android_msg})
      this.setState({Update_notnow:'0'})
  
    }
    else{
      this.setState({Update_msg:(Platform.OS == 'ios') ? jsonData.ios_msg : jsonData.android_msg})
      this.setState({Update_notnow:'1'})
      
    }
    var that = this;
    that.setState({Update_dialog:true})
   setTimeout(function(){ that.setState({Update_dialog:true})},1000)
    this.setState({loader:false})
      ...
      this.setState({Update_dialog:false})
...
var status = true;
 ...
 setTimeout(function(){ that.setState({ userStatusDialog: true });},500)
  ...
  showDialog(isShow: any, item: any,index) {
  ...
  async shareOption(item: any, selectedDatavalues: any, bettype: any,betValue:any) {
    ...
    if (bettype == 'ODDS') {
     ...
     var newWinAmount = '"'+this.state.POSTWinAmount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+'"'
      //  MessageString = "I’ll bet you the " +  newteamName +" beat the "+ newteamName2 +". The "+teamName+" are "+newLine+ " on the "+newoddsString+". I’ll put up " + newWinAmount + " UDDA bucks to your "+newAmount+" Udda bucks, you can accept all of or any part of my bet.";
...
//         beat the
//       <Text style={{ fontFamily: 'Montserrat-SemiBold' }}> {teamName2}</Text> The {teamName} are <Text style={{ fontFamily: 'Montserrat-SemiBold' }}>{drawOdds}</Text> on the <Text style={{ fontFamily: 'Montserrat-SemiBold'}}>{oddsString}</Text>. I’ll put up <Text style={{ fontFamily: 'Montserrat-SemiBold'  }}>{this.state.POSTWinAmount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} UDDA</Text> bucks to your <Text style={{ fontFamily: 'Montserrat-SemiBold'  }}>{this.state.POSTBetAmount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} UDDA</Text> bucks, you can accept all of or any part of my bet.
//   {referStr}</Text>
ShowString = <Text style={{ fontFamily: 'Montserrat-Regular', fontSize: hp(1.4), }}>
  ...
  {referStr}</Text>
      ...
      if (selectedData.MasterCalcFlag == 'D') {
        ...
        under 
      <Text style={{ fontFamily: 'Montserrat-SemiBold' }}> {oddsString}</Text> for  <Text style={{ fontFamily: 'Montserrat-SemiBold' }}>{teamName} </Text> Vs <Text style={{ fontFamily: 'Montserrat-SemiBold' }}>{teamName2}</Text>. I’ll put up <Text style={{ fontFamily: 'Montserrat-SemiBold' }}>{this.state.POSTBetAmount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} UDDA</Text> bucks to your <Text style={{ fontFamily: 'Montserrat-SemiBold' }}>{this.state.POSTWinAmount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} UDDA</Text> {" bucks, you can accept " + (this.state.POSTBetAmount >= 2000 ?"all of or any part of my bet ":"")+"or decline by tapping the link below."}.
  {referStr}</Text>
      }
 ...
 onChangeResult(value: any) {
  ...
  closeShareDialog() {
    ...
    //this.getCustomProbBetList(this.state.selectedBetData.event_id, this.state.selectedBetData.league_id)
    setTimeout(() => {
      //that.DialogClose(this.selectedItemData,this.selectedItemIndex);
      //AlertUtil.show("Bet Placed Successfully");
            ...
            <View style={{ flexDirection: 'row' }}>
              <View style={[styles.dropDown_Container, { alignContent: 'center', padding: 5 }]}>
                <Dropdown
                  ...
                  itemTextStyle={[styles.Input_TextStyle, { padding: 5, fontFamily: 'Montserrat-Bold', fontSize: hp(1.2), color: '#333333', margin: 0, paddingBottom: 0, width: '100%' }]}
                  data={this.data3}
                  value={this.state.selectedValue == 'all'?'ALL':this.state.selectedValue}
              ...
              <View style={[styles.dropDown_Container, { alignContent: 'center', padding: 5 }]}>
                <Dropdown
                  ...
                  value={this.state.selectedGameValue == 'all'?'ALL':this.state.selectedGameValue}
            ...
            <View style={{ flexDirection: 'row' }}>
              <View style={[styles.dropDown_Container, { alignContent: 'center', padding: 5 }]}>
                ...
                <Dropdown
                  ...
                  value={this.state.selectedSoccerValue== 'all'?'ALL':this.state.selectedSoccerValue}
            ...
            <View style={{ flexDirection: 'row' }}>
              <View style={[styles.dropDown_Container, { alignContent: 'center', padding: 5 }]}>
                ...
                <Dropdown
                  ...
                  itemTextStyle={[styles.Input_TextStyle, { padding: 5, fontFamily: 'Montserrat-Bold', fontSize: hp(1.2), color: '#333333', margin: 0, paddingBottom: 0, width: '100%' }]}
                  data={this.dataAutoRacing}
                  value={this.state.selectedAutoRacingValue == 'all'?'ALL':this.state.selectedAutoRacingValue}
            ...
            <View style={{ flexDirection: 'row' }}>
              <View style={[styles.dropDown_Container, { alignContent: 'center', padding: 5 }]}>
                ...
                <Dropdown
                  ...
                  value={this.state.selectedHorseRacingValue == 'all'?'ALL':this.state.selectedHorseRacingValue}
            ...
            <View style={{ flexDirection: 'row' }}>
              <View style={[styles.dropDown_Container, { alignContent: 'center', padding: 5 }]}>
                ...
                <Dropdown
                  ...
                  value={this.state.dashboard_team_id == 'all'?'ALL':this.state.dashboard_team_id}
        ...
        <View style={styles.dialog_container}>
          ...
          <View>
            <View style={{ flexDirection: 'row', width: '100%', marginTop: 3 }}>
              ...
              <View style={{ width: '50%' }}>
                <TouchableOpacity onPress={() => { this.showDialog(true, GamesArray,index) }}>
...
gotorefree(){
  this.props.navigation!.navigate(AppScreens.G_MyRefereesView,this.props);
  // RouterBuilder.replaceRouteTo(AppScreens.G_MyRefereesView,this.props);
}
  render() {
      ...
      <Container
        ...
        <ProgressLoader
          ...
          color={"#FFFFFF"} >
            <Dialog
                      // visible={true}
                     visible={this.state.Update_dialog}
                    ...
                    <View style={{ width: '100%' }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'flex-start', padding: 10,paddingLeft:20 }}>
                            <Text style={{ color: '#68bcbc', fontSize: hp(2.4), fontFamily: 'Montserrat-Bold' }}>Update Available</Text>
              ...
              {this.state.Update_msg}.</Text>
                  
                        </View>

                        <View style={{justifyContent:'center',alignContent:'center',alignItems:'center',width:'100%',marginBottom:8}}>
                      <TouchableOpacity onPress={()=>{this.updateAppNotice('UPDATE')}} style={{width:'90%',backgroundColor:'#68bcbc',padding:6,marginTop:5,paddingBottom:10,paddingTop:10}}>
                      <Text style={{color: '#fff', fontFamily: 'Montserrat-Bold', fontSize: hp(2.0),textAlign:'center' }}>UPDATE NOW</Text>
                      </TouchableOpacity>
                      {this.state.Update_notnow=='0'?<TouchableOpacity onPress={()=>{this.updateAppNotice('NOT')}} style={{width:'90%',backgroundColor:'#fff',padding:6,marginTop:5,paddingBottom:10,paddingTop:10}}>
          ...
          <View style={{ backgroundColor: "transparent",height:370,
              width:'100%' }}>
          ...
          Register Now & Receive
              ...
              <View style={{ width: '10%' }}>
                          <TouchableWithoutFeedback 
                           onPress={() => { this.setState({ guest2: false }) }}
                          >
                            <View>
          ...
          <View style={{ backgroundColor: "white",height:310,
              ...
              <Text style={{ fontFamily: 'Montserrat-Bold', fontSize: hp(5.8), textAlign:'center', color: '#5b5b5b',fontWeight:'bold' }}>{'10,000'}</Text>
          ...
          <View style={{marginTop:5}}>
              <Text style={{ fontFamily: 'Montserrat-Semibold', fontSize: hp(2.5) ,color:'#f26522',textDecorationLine:'underline',textDecorationColor:'#f26522',textAlign:'center' }}>
                Continue as Gues User
              </Text> 
              </View>
...
<SafeAreaView forceInset={{ bottom: 'never' }}>
         <ImageBackground source={require('../../../../images/dash_overlay2.png')}
                                resizeMode="stretch"
          ...
          <View style={{ backgroundColor: "transparent" }}>
            ...
            <View style={{ justifyContent: "center", backgroundColor: "#FFF" }} >
              <View style={{ justifyContent: "center", padding: 8, paddingHorizontal: 15 }}>
                ...
                {this.state.submitfeedbackbtn ? <BigButton title="SUBMIT"
        ...
        <Dialog
          ...
          title=""
          onTouchOutside={() => this.setState({ thanksDialog: false,submitfeedbackbtn:true })} >

          <View style={{ backgroundColor: "white" }}>

            <TouchableWithoutFeedback onPress={() => { this.setState({ thanksDialog: false,submitfeedbackbtn:true }) }} >
              <View>
                <Image source={require('../../../../images/close.png')} style={{ height: 13, width: 13, alignSelf: 'flex-end', marginRight: 8, marginTop: 10 }}></Image>
              </View>
            </TouchableWithoutFeedback>
 ...
 {/* -------------------------------- Bet Placed user Status check Dialogue --------------------------------*/}
        ...
        {this.state.guestUserDialog?<Guestdialog detailmodel={this.state.guestUserDialog} onDismiss={() => {
                  this.closeModal();
                   ...
                   {/* {this.state.guestUserDialog?<Otpdialog detailmodel={this.state.guestUserDialog} onDismiss={() => {
                  this.closeModal();
                }} />:null} */}
                {this.state.ReferDialog?<RefferDialog detailmodel={this.state.ReferDialog} gotorefree={() => {
          this.gotorefree();
        }} onDismiss={() => {
                  this.closeRefferModal();
                }} />:null}
...
<Dialog
        ...
        <View style={styles.scrollContent}>
          ...
          {  this.state.image_array.length>0 &&  <SliderBox
          dotStyle={{height:0,width:0}}
         autoplay={true}
         autoplayInterval={this.state.banner_time}
...
/>}
          ...
          {/* <Image source={{uri:'http://uddadev.triazinesoft.com/assets/banners/1603800947_minimum-bets.png'}} style={{ height: '100%', width: '100%'}} /> */}
            {/* <AdMobBanner
...
{/* {this.state.bingoDropDwon ? <View style={{backgroundColor:'#dddddd'}}>
              ...
              {/* {this.state.custonDropDown?null:<View style={{height:'8%',width:'100%',backgroundColor:'#DDDDDD',justifyContent:'center',alignContent:'center',alignItems:'center'}}>
                <TouchableOpacity onPress={()=>{this.gotofreetoplay()}}>
                <Text style={[{fontSize: hp(2.0),fontFamily: 'Montserrat-Bold',color:'#f26522',textDecorationLine:'underline',textDecorationColor:'#f26522',padding:5}]}>FREE TO PLAY CONTEST</Text>
                </TouchableOpacity>
              </View>} */}
