                    ...
                    {this.state.StandlingList.length > 0 ?
                        <View style={[styles.Flatlist_Style,{backgroundColor:(this.state.contesttype == 'free_to_play_contest')?'#FFF':'#dddddd'}]}>
                            {this.state.contesttype == 'free_to_play_contest'?this.state.responsedata.contest_result=='FINAL'?<View style={{width:'100%',flexDirection:'row',backgroundColor: '#666666',height:'15%'}}>
                                <View style={{width:this.state.responsedata.bankroll=='Y'&&this.state.responsedata.contest_result=='FINAL'?'37%':'40%',justifyContent:'center',alignContent:'center',alignItems:'center'}}>
                                <Text style={[styles.titleStyle]}>PARTICIPANT </Text>
                                </View>
                                <View style={{width:this.state.responsedata.bankroll=='Y'&&this.state.responsedata.contest_result=='FINAL'?'15%':'26%',justifyContent:'center',alignContent:'center',alignItems:'center'}}>
                                <Text style={[styles.titleStyle,{marginRight:this.state.responsedata.bankroll=='Y'?5:0}]}>{this.state.responsedata.bankroll=='Y'?'OPEN PICKS':'WINS'} </Text>
                                </View>
                                {this.state.responsedata.bankroll=='Y'&&<View style={{width:'21%',justifyContent:'center',alignContent:'center',alignItems:'center'}}>
                                <Text style={[styles.titleStyle]}>BANKROLL</Text>
                                </View>}
                                {<View style={{width:this.state.responsedata.bankroll=='Y'&&this.state.responsedata.contest_result=='FINAL'?'27%':'34%',justifyContent:'center',alignContent:'center',alignItems:'center'}}>
                                ...
                                <Text style={[styles.titleStyle,{marginRight:this.state.responsedata.bankroll=='Y'?5:0}]}>{this.state.responsedata.bankroll=='Y'?'PICKS':'OPEN PICKS'} </Text>
                                </View>
                                {/* {this.state.responsedata.bankroll=='Y'&&<View style={{width:'21%',justifyContent:'center',alignContent:'center',alignItems:'center'}}>
                                ...
                                <Text style={[styles.titleStyle,{textAlign:this.state.responsedata.bankroll=='Y'?'right':'auto'}]}>TOTAL WON </Text>
                                </View>}
                            </View>:null}
                                ...
                                renderItem={({ item, index }: any) => {
                                    ...
                                    return (
                                       ...
                                       {this.state.responsedata.contest_result=='FINAL'? <View style={{backgroundColor:'white',flexDirection:'row',borderBottomWidth:0.5,borderTopWidth: subindex==0?0.5:0,borderColor:'#dddddd'}}>
                                           <View style={{width:this.state.responsedata.bankroll=='Y'&&this.state.responsedata.contest_result=='FINAL'?'37%':'40%',justifyContent:'flex-start',alignContent:'center',alignItems:'center',flexDirection:'row',borderRightWidth:0.5,borderColor:'#dddddd',padding:3}}> 
                                      ...
                                      {item.type!=''&&item.is_win==0?<View style={{width:'50%',justifyContent:'center',alignContent:'center'}}>
                                                ...
                                                <View style={{width:this.state.responsedata.bankroll=='Y'?'15%':'26%',justifyContent:'center',alignContent:'center',alignItems:'center',flexDirection:'row',borderRightWidth:0.5,borderColor:'#dddddd'}}>
                                                ...
                                                <Text style={[styles.Count_Style,{color:item.is_win==0?'#666666':'#68bcbc',textDecorationLine:'underline'}]}><Text style={[styles.Name_Style,,{color:item.is_win==0?'#666666':'#68bcbc'}]}> {this.state.responsedata.bankroll=='Y'?item.won_picks:item.won_picks}</Text>
                                                    </Text>
                                                </TouchableOpacity>
                                                </View>
                                                ...
                                                <Text style={[styles.Count_Style,{color:item.is_win==0?'#666666':'#68bcbc',textDecorationLine:item.type!=''?'none':'none'}]}><Text style={[styles.Name_Style,,{color:item.is_win==0?'#666666':'#68bcbc'}]}> {item.available_bankroll}</Text>
                                                    </Text>
                                                </TouchableOpacity>
                                                </View>:null:null}
                                                ...
                                                <View style={{width:this.state.responsedata.bankroll=='Y'&&this.state.responsedata.contest_result=='FINAL'?'27%':'34%',justifyContent:'center',backgroundColor: item.Price == 'Lost'? '#bbbbbb':'#d2ebeb',alignContent:'center',alignItems:'center',flexDirection:'row',borderRightWidth:0.5,borderColor:'#dddddd'}}>
                                                    ...
                                                    // onPress={()=>{this.state.responsedata.bankroll=='Y'?null:this.goToBetList(item.userId,'won')}}
                                                    >
                                                 ...
                                                 {this.state.responsedata.bankroll=='Y' ? <Text style={[styles.Price_Text, { color:item.is_win==0?'#666666': '#68bcbc',textDecorationLine:'none' }]}>{item.Price}</Text>:<Text style={[styles.Price_Text, { color: item.is_win==0?'#666666':'#68bcbc',textDecorationLine:'none' }]}>{item.won_picks}</Text>}
                                                </View>
                                                </TouchableOpacity>
                                                </View>
                                           ...
                                           <View style={{width:this.state.responsedata.bankroll=='Y'&&this.state.responsedata.contest_result=='FINAL'?'33%':'40%',justifyContent:'flex-start',alignContent:'center',alignItems:'center',flexDirection:'row',borderRightWidth:0.5,borderColor:'#dddddd',padding:5}}> 
                                            <View style={[styles.Detail_Container1, { backgroundColor: 'white' }]}>
                                                    <View style={{ borderColor: '#666', borderRadius: 100, borderWidth: 1 }}>
                                                        <Image
                                                            style={{ width: 30, height: 30 }}
                                                            source={{ uri: item.Image }} >
                                                ...
                                                <View style={[styles.Name_Container1, { backgroundColor: 'white'}]}>
                                                {/* <TouchableOpacity onPress={() => {item.is_win==0?null:
                                        this.goToBetList(item.userId)
                                    }} > */}
                                    <View>
                                      <Text style={[styles.Count_Style,{color:item.is_win==0?'':'#68bcbc'}]}> {item.Name} {item.type!='' && <Text style={{fontSize:hp(1.6),color:item.is_win==0?'':'#68bcbc'}}>({item.type})</Text>}
                                                    </Text>
                                                    </View>
                                    ...
                                    {/* </TouchableOpacity>  */}
                                                ...
                                                <TouchableOpacity onPress={()=>{item.type!='' && this.goToBetList(item.userId,'')}} style={{width:'100%',justifyContent:'center',alignItems:'center'}}>
                                                <Text style={[styles.Count_Style,{color:item.is_win==0&&item.type==''?'#666666':'#68bcbc',textDecorationLine:item.type!=''?'underline':'none'}]}><Text style={[styles.Name_Style,,{color:item.is_win==0&&item.type==''?'':'#68bcbc'}]}> {this.state.responsedata.bankroll=='Y'?item.picks:item.open_picks}</Text>
                                                    </Text>
                                                </TouchableOpacity>
                                                </View>
