...
interface G_SummaryNotificationViewProps extends AppValidationComponentProps { }

interface G_SummaryNotificationViewState extends AppValidationComponentState {
    DataList: any;
    Flag: any;
    loader: any;
    expandText: boolean;
...
export default class G_SummaryNotificationView extends AppValidationComponent<G_SummaryNotificationViewProps, G_SummaryNotificationViewState>
    implements MenuIconListener, LogoIconListener {
    private FAQData: any
    private authorisationToken = Application.sharedApplication().user!.authenticationToken;
    private referralservice = new ReferralService();
    constructor(props: G_SummaryNotificationViewProps) {
        super(props);
        this.state = {
            DataList: '',
            Flag: 'A',
    ...
    componentDidMount() {
		...
		let summary_date = navigation.getParam("summary_date");
		if(summary_date!=''){
            this.getNotificationList(summary_date);
            this.setState({summary_date:summary_date});
		}else{
			AlertUtil.show('The summary date is required');
		}
    ...
    getNotificationList(summary_date:any) {
        ...
        fetch(UrlService.CONSTURI + 'index.php/'+ UrlService.APIVERSION3 +'/PushNotification/get_summary_notifications', {
            method: 'POST',
            headers: {
            ...
            .then((responseJson) => {
				console.log('respoooooo ', JSON.stringify(responseJson));
                if (responseJson.error == '0') {
					this.setState({ notificationList: responseJson.data });

                    console.log('notificationssssssssssssss: ', JSON.stringify(this.state.notificationList));
				}else{
    ...
    deleteNotificationList(noti_id) {
        ...
        fetch(UrlService.CONSTURI + 'index.php/'+ UrlService.APIVERSION3 +'/PushNotification/delete_push_notification', {
            ...
            .then((responseJson) => {
                this.getNotificationList(this.state.summary_date);
            })
        .catch(error => {
            this.setState({ loader: false });
            console.log(error); 
...
switch (item.page_name) {
    ...
    case 'private_bet_list':
        this.props.navigation!.replace(AppScreens.G_GamingBetView,{params:{bet_id:'private'}});
            break;
    case 'f2p_winners':
       this.props.navigation!.replace(AppScreens.G_StandingList , {params:{bet_id:item.action_id,type:'free_to_play_contest'}});
           break;
    case 'profile':
        this.props.navigation!.replace(AppScreens.G_ProfileView , this.props);
         break;
    ...
    render() {  
        ...
        return (
            ...
            <View style={{ flex: 1, alignContent: 'center', alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ fontFamily: 'Montserrat-Bold', fontSize: hp(2.3), color: '#68bcbc', textAlign: 'center', marginTop: 7, alignContent: 'center' }}>NOTIFICATION SUMMARY</Text>
            </View>
          </View>
          <View style={{ marginBottom: '5%', borderBottomColor: '#BBBBBB', borderBottomWidth: 1 }}></View>
                                    ...
                                    <FlatList
                                        ...
                                        return (
                                            <View style={[styles.listItem,{marginBottom: '4%',paddingBottom: '2%'}]}>
                                                ...
                                                {item.action_required == 0 ?  null : <Text style={[styles.viewMore, { paddingHorizontal: '2%', paddingVertical: '1%' ,textAlign:'right'}]} onPress={() => {
                                                            this.clickPushNotificationNav(item)
                                                        }}>{item.button_name}</Text>}
                                                </View>
