...
class ContestDashboardView extends AppValidationComponent<G_ContestDashboardViewProps, G_ContestDashboardViewState>
      ...
      if (numberofRows == 'All') {
        if (from_private_contest == "1") {
          var params: any = {
            ...
            'contest_type': 'private_free_to_play'
          };
        } else {
          var params: any = {
            'league_id': data[0][1],
            ...
            'per_page': '',
            'timezone': deviceTimeZone,
            'contest_type': data[4][1] == 'free_to_play_contest' ? 'free_to_play' : 'public_contest'
          };
        }
      } else {
        if (from_private_contest == "1") {
            ...
            switch (nextProps.requestStatus) {
              case ServiceRequestStatus.FinishedWithSuccess:
                ...
                if (response.message == 'success') {
                  ...
                  if (this.state.contestdata.bankroll == 'Y') {
                    ...
                    var that = this;
                    var data = that.state.contestdata;
                    data.avaiable_bankroll_amount = response.data.available_bankroll_amount;
                    data.rejoin_btn = response.data.rejoin_btn;
                    data.rentry_amount = response.data.rentry_amount;
                  ...
                  if (response.data.type == '1') {
                    if (!isPublickAlertOn) {
                      ...
                      AlertUtil.showSingleActionMessage("Bet Placed Successfully", function () {
                        
                          if (response.data.rejoin_btn) {

                            that.setState({ reenterdialog: true });
  ...
  async rejoinContest() {
    //  console.log('rejoin contesttype public_contest: ',this.state.contesttype)
    //  return;

    var url;
    if (this.state.contesttype == 'public_contest') {
      url = UrlService.CONSTURI + 'index.php/' + UrlService.APIVERSION3 + '/publicContest/rejoin_public_contest'
    } else if (this.state.contesttype == 'private_contest') {
      url = UrlService.CONSTURI + 'index.php/' + UrlService.APIVERSION3 + '/PrivateContest/rejoin_private_contest'
    } else {
      url = UrlService.CONSTURI + 'index.php/' + UrlService.APIVERSION3 + '/apiGaming/rejoin_free_to_play_contest'
    }
    ...
    if (this.state.contesttype == 'private_contest') {
      ...
      var params: any = {
        ...
        'rejoin_amount': this.state.contestdata.rentry_amount
      };
    } else {
      var params: any = {
        'rejoin_amount': this.state.contestdata.rentry_amount,
    ...
    console.log('prams rejoin : ', params)
    console.log('url rejoin : ', url)
    var that = this;
    var formData = new FormData();
    ...
    fetch(url, {
      ...
      .then((responseJson) => {
        ...
        AlertUtil.showSingleActionMessage(responseJson.message, function () {
          that.setState({ reenterdialog: false });
        })
        var data = that.state.contestdata
        data.avaiable_bankroll_amount = responseJson.data.avaiable_bankroll_amount
        data.rejoin_btn = responseJson.data.rejoin_btn
  ...
  render() {
    ...
    return (
      <Container
        ...
        <Dialog
          //   visible={true}
          visible={this.state.reenterdialog}
          ...
          onTouchOutside={() => this.setState({ reenterdialog: false })} >
          <View style={{ width: '100%' }}>
            <View style={{ flexDirection: 'row', justifyContent: 'center', padding: 10 ,position:'relative'}}>
              <Text style={{ color: '#68bcbc', fontSize: hp(2.4), fontFamily: 'Montserrat-Bold' }}>RE-ENTER CONTEST</Text>
              <View style={{position:'absolute',right:'5%',top:'60%'}}>
              <TouchableOpacity onPress={() => { this.setState({ reenterdialog: false }) }} >
                <Image source={require('../../../../images/close.png')}  resizeMode='contain' style={{ width: wp(4), height: wp(4), tintColor: 'grey'}}/>
              </TouchableOpacity>
              </View>
            </View> 
            ...
            <View style={{ width: '100%', padding: 8, backgroundColor: '#dddddd', marginTop: 5, justifyContent: 'center', alignContent: 'center', alignItems: 'center', flexDirection: 'row' }}>
              <Text style={{ color: 'black', fontFamily: 'Montserrat-SemiBold', fontSize: hp(2.0) }}>Amount to Re-enter : </Text>
              <Image source={require('../../../../images/buck_dark.png')} style={{ width: wp(4), height: wp(4), marginTop: 1 }} resizeMode='contain' />
              <Text style={{ color: 'black', fontFamily: 'Montserrat-Bold', fontSize: hp(2.8), marginTop: 1 }}>{this.state.contestdata.rentry_amount}</Text>
            </View>

            <View style={{ justifyContent: 'center', alignContent: 'center', alignItems: 'center', padding: 15 }}>
              <Text style={{ color: '#1c1c1c', fontFamily: 'Montserrat-Bold', fontSize: hp(2.8), textAlign: 'center' }}>{'Re-entry amount will be deducted from your available UDDA Bucks'}</Text>
            </View>
            <View style={{ justifyContent: 'center', alignContent: 'center', alignItems: 'center', width: '100%' }}>
              <TouchableOpacity onPress={() => { this.rejoinContest() }} style={{ width: '90%', backgroundColor: '#68bcbc', padding: 6, marginTop: 5, paddingBottom: 10, paddingTop: 10 }}>
                <Text style={{ color: '#fff', fontFamily: 'Montserrat-Bold', fontSize: hp(2.0), textAlign: 'center' }}>PROCEED</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => { this.setState({ reenterdialog: false }) }} style={{ width: '90%', backgroundColor: '#fff', padding: 6, marginTop: 5, marginBottom: 8, paddingBottom: 10, paddingTop: 10 }}>
                <Text style={{ color: '#72bfbf', fontFamily: 'Montserrat-Bold', fontSize: hp(2.0), textAlign: 'center' }}>CANCEL</Text>
              </TouchableOpacity>
            </View>
        ...
        <View style={styles.scrollContent}>
          ...
          <View style={[styles.mainContent, { width: '100%' }]}>
            ...
            <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center' }}>
              <View style={{ width: '100%', justifyContent: 'center', flexDirection: 'row' }}>
                ...
                <View style={{ width: '100%', flexDirection: 'row', backgroundColor: '#CCCCCC' }}>
                  ...
                  <View style={{ width: '76%', justifyContent: 'center', alignItems: 'center', padding: 5 }}>
            ...
            {this.state.contestdata.bankroll == 'Y' ? <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', alignContent: 'center', marginBottom: 10, flexDirection: 'row' }}>
              ...
              {this.state.contestdata.rejoin_btn &&
                <TouchableOpacity style={{ marginLeft: 7, marginTop: 4 }} onPress={() => { this.setState({ reenterdialog: true }) }}>
                  <View style={{ justifyContent: 'center', backgroundColor: '#fe9700', padding: 6 }}>
                    <Text style={{ textAlign: 'center', color: 'white', fontFamily: 'Montserrat-Bold', fontSize: hp(1.6) }}>RE-ENTER</Text>
                  </View>
                </TouchableOpacity>
              }
            </View> : null}
